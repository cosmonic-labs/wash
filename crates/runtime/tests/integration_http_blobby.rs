...
use runtime::{
    ...
    plugin::{http_server::HttpServer, wasi_blobstore::WasiBlobstore, wasi_logging::WasiLogging},
};

const BLOBBY_WASM: &[u8] = include_bytes!("../blobby.wasm");
...
async fn test_blobby_integration() -> Result<()> {
    ...
    assert!(
        ...
        "GET request expected success or 404, got {}", get_status
    );
    assert!(post_status.is_success(), "POST request expected success, got {}", post_status);
    assert!(
...
async fn test_blobby_error_handling() -> Result<()> {
    ...
    let host = host.start().await.context("Failed to start host for error test")?;
