...
use runtime::{
    Component, Engine, HostApi, HostBuilder, LocalResources, WitInterface, Workload,
    host::WorkloadStartRequest,
    plugin::{
        http_server::HttpServer, runtime_config::RuntimeConfig, wasi_blobstore::WasiBlobstore,
        wasi_keyvalue::WasiKeyvalue, wasi_logging::WasiLogging,
    },
};

const HTTP_COUNTER_WASM: &[u8] = include_bytes!("../http_counter.wasm");
...
async fn test_http_counter_integration() -> Result<()> {
    ...
    // Create HTTP server plugin on a specific port
        ...
        workload: Workload {
            ...
            host_interfaces: vec![
                ...
                WitInterface {
                    ...
                    interfaces: vec!["store".to_string(), "atomics".to_string()],
                    version: Some(semver::Version::parse("0.2.0-draft").unwrap()),
                    config: HashMap::new(),
                },
                WitInterface {
...
async fn test_http_counter_error_scenarios() -> Result<()> {
    ...
    let engine = Engine::builder().build()?;
    let addr: SocketAddr = "127.0.0.1:8089".parse().unwrap();
    let http_plugin = HttpServer::new(addr);
    let blobstore_plugin = WasiBlobstore::new(None);
    let keyvalue_plugin = WasiKeyvalue::new();
    let logging_plugin = WasiLogging {};
        ...
        workload: Workload {
            ...
            components: vec![Component {
                ...
                local_resources: LocalResources {
                    ...
                    config: HashMap::new(),
                    volume_mounts: vec![],
                    allowed_hosts: vec![], // No outbound hosts allowed
                },
                pool_size: 1,
                max_invocations: 50,
            }],
...
async fn test_http_counter_plugin_isolation() -> Result<()> {
    ...
    let addr1: SocketAddr = "127.0.0.1:8090".parse().unwrap();
    let addr2: SocketAddr = "127.0.0.1:8091".parse().unwrap();
